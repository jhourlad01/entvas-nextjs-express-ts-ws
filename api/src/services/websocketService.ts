...
class WebSocketService {
  ...
  private async sendStatsToClient(client: ConnectedClient) {
    try {
      ...
      // Calculate events per minute for the last hour (including current minute)
      const eventsPerMinute: Array<{timestamp: string; count: number}> = [];
      
      // Round down to the nearest minute for consistent time windows
  ...
  async broadcastStats() {
    ...
    try {
      ...
      for (let i = 0; i < 60; i++) {
        const minuteStart = new Date(now.getTime() - (59 - i) * 60 * 1000);
        const minuteEnd = new Date(minuteStart.getTime() + 60 * 1000);
